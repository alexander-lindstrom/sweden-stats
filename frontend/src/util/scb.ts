...
export interface JsonStat2Response {
  class: "dataset"; version: "2.0"; label?: string;
  source?: string; updated?: string;
  href?: string; id: string[]; size: number[];
...
const DEFAULT_SELECTIONS = {
  ...
  [VARIABLE_CODE_MAP.ages]: { codeList: "vs_Ålder1årA", valueCodes: ["1", "2", "3"] } // can use ["1", "2", "3", ...]
};
...
export function buildScbApiRequestBody(args: PopulationQueryArgs): { selection: Selection[] } {
  const selections: Selection[] = [];

  if (args.regionCodes?.length) {
